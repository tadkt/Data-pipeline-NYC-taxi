...
def load_minio_config(spark_context: SparkContext):
    ...
    Args:
        spark_context (SparkContext): Establish the necessary config to access to MinIO
    """
    try:
        spark_context._jsc.hadoopConfiguration().set("fs.s3a.access.key", MINIO_ACCESSKEY)
        spark_context._jsc.hadoopConfiguration().set("fs.s3a.secret.key", MINIO_SECRETKEY)
...
def processing_dataframe(df, file_path):
    ...
    df2 = df.withColumn('year', F.year('pickup_datetime')) \
            ...
            .withColumn('year', F.date_format('pickup_datetime', 'EEEE'))
...
if __name__ == "__main__":
    ...
    for file in client.list_parquet_files(BUCKET_NAME, prefix = 'batch/'):
        ...
        logging.info(f"Reading parquet files: {file}")

        df = spark.read.parquet(path)
        df_final = processing_dataframe(df, file)

        # Load data to staging table at PostgreSQL
